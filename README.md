...
The motivation for establishing the DISO repository stems from the research problem of OWL **ontology alignment**. Using DISO, the aim is to support a **defence, intelligence & security track** for the [Ontology Alignment Evaluation Initiative (OAEI)](https://oaei.ontologymatching.org/).  OAEI tracks contain one or more **ontology alignment tasks**. An OAEI ontology alignment task involves a pair of ontologies whose domains overlap (or abut) in a way that presents interesting alignment opportunities and challenges. The pair of ontologies is accompanied by a **reference alignment** (a ground-truth alignment).  The **reference alignment** permits the quality (accuracy, recall, precision, and related measures) of alignments generated by **matching systems** (ontology alignment software systems) to be evaluated.
...
| Establish preliminary reference alignments for the final set of alignment tasks | COMPLETE    |
